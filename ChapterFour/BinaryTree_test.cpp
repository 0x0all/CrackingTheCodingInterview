...
public:
	BinaryTreeTest() :
			_fullIntBinaryTreeTwoLevels(nullptr), _fullIntBinaryTreeTwoLevelsWithParentLinks(
					nullptr) {
	}
protected:
	virtual void SetUp() {
		_fullIntBinaryTreeTwoLevels = CreateFullIntTreeTwoLevels();
		_fullIntBinaryTreeTwoLevelsWithParentLinks =
				CreateFullIntTreeTwoLevelsWithParentLinks();
	...
	static unique_ptr<BinaryTree<int>> CreateFullIntTreeTwoLevels() {
		unique_ptr<BinaryTree<int>> fullTree(new BinaryTree<int>(8));
		fullTree->addNode(6);
		fullTree->addNode(10);
		fullTree->addNode(5);
		fullTree->addNode(7);
		fullTree->addNode(9);
		fullTree->addNode(11);
		return move(fullTree);
	}

	static unique_ptr<BinaryTree<int>> CreateFullIntTreeTwoLevelsWithParentLinks() {
		unique_ptr<BinaryTree<int>> fullTree(new BinaryTree<int>(8));
	...
	template<class T>
	static bool NodeNotNullWithExpectedValue(T expValue, const TreeNode<T>* node) {
	...
	template<class T>
	static bool NodeValuesAreTheExpected(const vector<TreeNode<T>*> &nodes, const vector<T> &expValues) {
	...
	unique_ptr<BinaryTree<int>> _fullIntBinaryTreeTwoLevels;
	unique_ptr<BinaryTree<int>> _fullIntBinaryTreeTwoLevelsWithParentLinks;
...
TEST_F(BinaryTreeTest, AddRightNode) {
	...
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(2,
					root->rightChild));
}
...
TEST_F(BinaryTreeTest, AddLeftNode) {
	...
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(0,
					root->leftChild));
}
...
TEST_F(BinaryTreeTest, AddRightRightChild) {
	...
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(5,
...
TEST_F(BinaryTreeTest, AddLeftRightNode) {
	...
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(7,
...
TEST_F(BinaryTreeTest, InOrderTraversal) {
	...
	ASSERT_TRUE(
			BinaryTreeTest::VectorsAreEqual<int>(result,
					_inOrderTraversalResult));
}
...
TEST_F(BinaryTreeTest, PreOrderTraversal) {
	...
	ASSERT_TRUE(
			BinaryTreeTest::VectorsAreEqual<int>(result,
					_preOrderTraversalResult));
}

TEST_F(BinaryTreeTest, PostOrderTraversal) {
	vector<int> result =
			_fullIntBinaryTreeTwoLevels->postOrderTraversalValues();

	ASSERT_TRUE(
			BinaryTreeTest::VectorsAreEqual<int>(result,
					_postOrderTraversalResult));
}

TEST_F(BinaryTreeTest, LevelOrderTraversal) {
	vector<int> result =
			_fullIntBinaryTreeTwoLevels->levelOrderTraversalValues();

	ASSERT_TRUE(
			BinaryTreeTest::VectorsAreEqual<int>(result,
					_levelOrderTraversalResult));
}
...
TEST_F(BinaryTreeTest, CreationWithVectorEven) {
	...
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(2,
					root->rightChild));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(3,
					root->rightChild->rightChild));
}
...
TEST_F(BinaryTreeTest, CreationWithVectorOdd) {
	...
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(0,
					root->leftChild));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(1,
					root->leftChild->rightChild));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(3,
					root->rightChild));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(4,
					root->rightChild->rightChild));
}

TEST_F(BinaryTreeTest, NodesPerLevel) {
	map<int, vector<TreeNode<int>*> > result =
			_fullIntBinaryTreeTwoLevels->getNodesPerLevel();
	ASSERT_TRUE(
			BinaryTreeTest::NodeValuesAreTheExpected<int>(result[0],
					_levelZeroNodeValues));
	ASSERT_TRUE(
			BinaryTreeTest::NodeValuesAreTheExpected<int>(result[1],
					_levelOneNodeValues));
	ASSERT_TRUE(
			BinaryTreeTest::NodeValuesAreTheExpected<int>(result[2],
					_levelTwoNodeValues));
}

TEST_F(BinaryTreeTest, AddNodeNodesWithParent) {
	auto root = _fullIntBinaryTreeTwoLevelsWithParentLinks->getRootPtr();

	ASSERT_TRUE(BinaryTreeTest::NodeNotNullWithExpectedValue<int>(8, root));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(10,
					root->rightChild));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(8,
					root->rightChild->parent));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(6,
					root->leftChild));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(8,
					root->leftChild->parent));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(11,
					root->rightChild->rightChild));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(10,
					root->rightChild->rightChild->parent));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(9,
					root->rightChild->leftChild));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(10,
					root->rightChild->leftChild->parent));
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(7,
					root->leftChild->rightChild));
	ASSERT_TRUE(
	...
	ASSERT_TRUE(
			BinaryTreeTest::NodeNotNullWithExpectedValue<int>(6,
					root->leftChild->leftChild->parent));
}

TEST_F(BinaryTreeTest, InOrderNext) {
	map<int, vector<TreeNode<int>*> > result =
			_fullIntBinaryTreeTwoLevelsWithParentLinks->getNodesPerLevel();

	ASSERT_EQ(9,
			_fullIntBinaryTreeTwoLevelsWithParentLinks->inOrderNext(
					*result[0].at(0))->value);
	ASSERT_EQ(7,
			_fullIntBinaryTreeTwoLevelsWithParentLinks->inOrderNext(
					*result[1].at(0))->value);
...
TEST_F(BinaryTreeTest, CommonAncestor) {
	const TreeNode<int> *root =
			_fullIntBinaryTreeTwoLevelsWithParentLinks->getRootPtr();
	map<int, vector<TreeNode<int>*> > result =
			_fullIntBinaryTreeTwoLevelsWithParentLinks->getNodesPerLevel();
	const TreeNode<int> *commonAncestor = BinaryTree<int>::commonAncestor(*root,
			*result[2].at(1), *result[1].at(1));
...
TEST_F(BinaryTreeTest, ContainsSubtreeIdenticalTrees) {
	ASSERT_TRUE(
			BinaryTree<int>::containsSubTree(
					*_fullIntBinaryTreeTwoLevelsWithParentLinks,
					*_fullIntBinaryTreeTwoLevelsWithParentLinks));
}
...
TEST_F(BinaryTreeTest, ContainsSubtreeTrue) {
	...
	ASSERT_TRUE(
			BinaryTree<int>::containsSubTree(
					*_fullIntBinaryTreeTwoLevelsWithParentLinks, smallTree));
}
...
TEST_F(BinaryTreeTest, FindAllPathsToSum) {
	vector<vector<TreeNode<int>*>> result = BinaryTree<int>::findAllPathsToSum(
			*_fullIntBinaryTreeTwoLevelsWithParentLinks, 19);
