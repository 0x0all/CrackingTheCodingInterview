...
TEST(GraphNodeTest, AddAdjacent){
	GraphNode<int> node(1);
	node.adjacent.push_back(std::shared_ptr<GraphNode<int>>(new GraphNode<int>(2)));
	node.adjacent.push_back(std::shared_ptr<GraphNode<int>>(new GraphNode<int>(3)));
...
TEST(GraphNodeTest, EmplaceMakeShared){
	GraphNode<int> node(1);
	node.adjacent.emplace_back(std::make_shared<GraphNode<int>>(2));
	node.adjacent.emplace_back(std::make_shared<GraphNode<int>>(3));
	ASSERT_EQ(2, node.adjacent[0]->value);
	ASSERT_EQ(3, node.adjacent[1]->value);
}
